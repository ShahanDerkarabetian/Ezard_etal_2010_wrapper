...
##import data
table <- read.table("example_data.csv", sep=",")
spps <- read.table("example_taxon.csv", sep=",")
rows <- nrow(table)
cols <- ncol(table)
tt <- matrix(scan("example_data.csv", sep=",", n=as.numeric(rows)*as.numeric(cols)), as.numeric(rows), as.numeric(cols), byrow=TRUE)
